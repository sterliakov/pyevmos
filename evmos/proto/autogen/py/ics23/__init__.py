...
class ExistenceProof(betterproto.Message):
    ...
    *ExistenceProof takes a key and a value and a set of steps to perform on
    ...
    infeasible to produce a hash collission for any ofthe used cryptographic
    hash functions, if someone can provide a series ofoperations to transform a
    given key and value into a root hash that matches sometrusted root, these
    key and values must be in the referenced merkle tree.The only possible
    issue is maliablity in LeafOp, such as providing extra prefixdata, which
    should be controlled by a spec. Eg. with lengthOp as NONE, prefix =FOO, key
    = BAR, value = CHOICE and prefix = F, key = OOBAR, value = CHOICE
    wouldproduce the same value.With LengthOp this is tricker but not
    impossible. Which is why the"leafPrefixEqual" field in the ProofSpec is
    valuable to prevent this mutability.And why all trees should length-prefix
...
class LeafOp(betterproto.Message):
    ...
    *LeafOp represents the raw key-value data we wish to prove, andmust be
    ...
    needed (ANY means no hashin this case): hkey = prehashKey(key) hvalue =
...
class InnerOp(betterproto.Message):
    ...
    *InnerOp represents a merkle-proof step that is not a leaf.It represents
    concatenating two children and hashing them to provide the nextresult.The
    ...
    length, or prepending theentire operation with some value to differentiate
    from leaf nodes, should beincluded in prefix and suffix. If either of
    prefix or suffix is empty, we justtreat it as an empty string
    """
...
class InnerSpec(betterproto.Message):
    ...
    proofsfrom a given store are neighbors.This enables:isLeftMost(spec:
...
class CompressedBatchEntry(betterproto.Message):
    ...
    nonexist: CompressedNonExistenceProof = betterproto.message_field(2, group='proof')
