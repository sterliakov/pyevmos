...
class HashOp(betterproto.Enum):
    NO_HASH = 0
    """
    NO_HASH is the default if no data passed. Note this is an illegal argument some
    places.
    """
...
class LengthOp(betterproto.Enum):
    ...
    *
    ...
    REQUIRE_32_BYTES = 7
    """
    REQUIRE_32_BYTES is like NONE, but will fail if the input is not exactly 32 bytes
    (sha256 output)
    """

    REQUIRE_64_BYTES = 8
    """
    REQUIRE_64_BYTES is like NONE, but will fail if the input is not exactly 64 bytes
    (sha512 output)
    """
...
class ExistenceProof(betterproto.Message):
    ...
    *
    ...
    Since it is computationally infeasible to produce a hash collission for any of the
    used
    cryptographic hash functions, if someone can provide a series of operations to
    transform
    a given key and value into a root hash that matches some trusted root, these key and
    values
...
class CommitmentProof(betterproto.Message):
    """
    CommitmentProof is either an ExistenceProof or a NonExistenceProof, or a Batch of
    such messages
    """
...
class LeafOp(betterproto.Message):
    ...
    *
    ...
    To process this, first prehash the keys and values if needed (ANY means no hash in
    this case):
    hkey = prehashKey(key)
...
class InnerOp(betterproto.Message):
    ...
    *
    ...
    Any special data, like prepending child with the length, or prepending the entire
    operation with
    some value to differentiate from leaf nodes, should be included in prefix and
    suffix.
...
class ProofSpec(betterproto.Message):
    ...
    *
    ...
    """
    max_depth (if > 0) is the maximum number of InnerOps allowed (mainly for fixed-depth
    tries)
    ...
    """
    min_depth (if > 0) is the minimum number of InnerOps allowed (mainly for fixed-depth
    tries)
    """
...
class InnerSpec(betterproto.Message):
    """
    InnerSpec contains all store-specific structure info to determine if two proofs from
    a
    ...
    """
    empty child is the prehash image that is used when one child is nil (eg. 20 bytes of
    0)
    """
