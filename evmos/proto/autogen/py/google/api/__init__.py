...
class HttpRule(betterproto.Message):
    ...
    The same http annotation can alternatively be expressed inside the `GRPC
    API Configuration` YAML file.     http:       rules:         - selector:
...
class HttpBody(betterproto.Message):
    ...
    extensions: list[betterproto_lib_google_protobuf.Any] = betterproto.message_field(3)
    """
